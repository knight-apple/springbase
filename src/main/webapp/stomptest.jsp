...
<script>
    ...
    stomp.connect(headers, function (frame) {
        ...
        stomp.subscribe("/users/queue/shouts",function (message) {
            var content = message.body;
            var obj = JSON.parse(content);
            console.log("admin1：" + obj.message);
        });
        /*第二种*/
        stomp.send("/app/shout", {}, adminJSON);
        stomp.subscribe("/users/queue/notifications",function (message) {
            var content = message.body;
            var obj = JSON.parse(content);
            console.log("admin2：" + obj.message);
        });

        /*订阅异常消息*/
        stomp.subscribe("/user/admin/queue/errors", function (message) {
